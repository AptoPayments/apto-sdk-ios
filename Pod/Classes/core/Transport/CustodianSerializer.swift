...
extension Custodian {
    var asJson: [String: AnyObject] {
        if let credentials = externalCredentials, case let .oauth(oauthCredentials) = credentials {
        ...
        if let credentials = externalCredentials {
...
extension ExternalCredential {
    var asJson: [String: AnyObject] {
        ...
        case .none:
            return [:]
        case let .oauth(oauthCredential):
            return oauthCredential.asJson
        case let .externalOauth(externalOauthCredential):
