...
extension JSON {
    public var linkObject: Any? {
        ...
        case "required_datapoint":
            return requiredDatapoint
        case "product":
            return product
        case "action":
            return workflowAction
        case "action_generic_message_config":
            return showGenericMessageWorkflowActionConfiguration
        case "call_to_action":
            return callToAction
        case "action_collect_user_data_config":
            return collectUserDataActionConfiguration
        case "action_disclaimer_config":
            return disclaimerActionConfiguration
        case "merchant":
            return merchant
        case "store":
            return store
        ...
        case "phone_datapoint_configuration":
            return allowedCountriesRequiredDataPointConfig
        case "address_datapoint_configuration":
            return allowedCountriesRequiredDataPointConfig
        case "id_document_datapoint_configuration":
            return allowedIdDocumentTypesRequiredDataPointConfig
        case "ivr":
            return ivr
        case "physical_activation_result":
            return physicalCardActivationResult
        case "allowed_balance_type":
            return allowedBalanceType
        case "select_balance_store_configuration":
            return selectBalanceStoreActionConfiguration
        case "action_issue_card_config":
            return issueCardActionConfiguration
        case "category_spending":
            return categorySpending
        case "monthly_spending":
            return monthlySpending
        case "monthly_statement_report":
            return monthlyStatementReport
        case "month":
            return month
        case "monthly_statements_period":
            return monthlyStatementsPeriod
        case "action_waitlist_config":
            return waitListActionConfiguration
    ...
    var projectBranding: ProjectBranding? {
        guard let iconPrimaryColor = self["icon_primary_color"].string,
              ...
              let uiTheme = self["ui_theme"].string
        else {
            return nil
        }
        let logoUrl = self["logo_url"].string
    ...
    var projectConfiguration: ProjectConfiguration? {
        guard let name = self["name"].string,
              ...
              let branding = self["branding"].branding
        else {
            ErrorLogger.defaultInstance().log(error: ServiceError(code: ServiceError.ErrorCodes.jsonError,
    ...
    var cardApplication: CardApplication? {
        guard let id = self["id"].string, let rawStatus = self["status"].string,
              ...
              let workflowObjectId = self["workflow_object_id"].string
        else {
            ErrorLogger.defaultInstance().log(error: ServiceError(code: ServiceError.ErrorCodes.jsonError,
    ...
    var transaction: Transaction? {
        ...
        let createdAt = Date(timeIntervalSince1970: rawCreatedAt)
    ...
    var fundingSource: FundingSource? {
        guard let fundingSourceId = self["id"].string,
              ...
              let rawState = self["state"].string, let state = FundingSourceState(rawValue: rawState)
        else {
            ErrorLogger.defaultInstance().log(error: ServiceError(code: ServiceError.ErrorCodes.jsonError,
    ...
    var oauthAttempt: OauthAttempt? {
        guard let id = self["id"].string, let rawStatus = self["status"].string,
              let status = OauthAttempt.Status(rawValue: rawStatus)
        else {
            ErrorLogger.defaultInstance().log(error: ServiceError(code: ServiceError.ErrorCodes.jsonError,
