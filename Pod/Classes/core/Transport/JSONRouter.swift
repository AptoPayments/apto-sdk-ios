...
class URLWrapper: URLConvertible {
    ...
    public func asURL() throws -> URL {
        var trailing: String = ""
        if let urlTrailing = urlTrailing {
            trailing = "/" + urlTrailing
        }

        guard let urlParameters = urlParameters else {
            return URL(string: baseUrl + self.url.URLString + trailing)! // swiftlint:disable:this force_unwrapping
        }
        let url = self.url.URLString.replace(urlParameters)
        let trailingParameters = urlParameters.urlRepresentation
        // swiftlint:disable:next force_unwrapping
        return URL(string: baseUrl + url + trailing + (!trailingParameters.isEmpty ? "?" + trailingParameters : ""))!
    }
}
