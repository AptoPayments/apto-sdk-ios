...
public extension CardPaymentSource {
    var title: String? {
        guard let cardNetwork = network?.description(), let cardType = cardType else {
            return nil
        }
...
extension JSON {
    ...
    func paymentSource(with key: String = "payment_source") -> PaymentSource? {
        guard let paymentSource = self[key].dictionary,
              let type = paymentSource["type"]?.string,
              let paymentSourceType = PaymentSourceType(rawValue: type)
        else {
    ...
    private func cardPaymentSource(with key: String = "payment_source") -> PaymentSource? {
        ...
        guard let id = paymentSource["id"].string,
              let lastFour = paymentSource["last_four"].string,
              let isPreferred = paymentSource["is_preferred"].bool
        else {
            ErrorLogger.defaultInstance().log(error: ServiceError(code: ServiceError.ErrorCodes.jsonError,
                                                                  reason: "Can't parse PaymentSource \(self)"))
            return nil
