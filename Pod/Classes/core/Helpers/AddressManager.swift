...
open class AddressManager {
    ...
    open func validate(address: Address, result: @escaping Result<AptoGeocoderPlace, NSError>.Callback) {
        geocode(address: address) { locations in
            switch locations {
    ...
    open func statesFor(country: String) -> [State]? {
        return stateStorage.getStates(country: country)
    }

    open func getStateBy(_ country: String, isoCode: String) -> State? {
        return stateStorage.getStateBy(country, isoCode: isoCode)
    }

    open func getStateBy(_ country: String, name: String) -> State? {
        return stateStorage.getStateBy(country, name: name)
    }
        ...
        query.fetchPlaces { places, error in
            if let places = places as? [GKPlaceAutocomplete], !places.isEmpty {
                let retVal = places.compactMap {
                    Place(id: $0.placeId, name: $0.textDescription)
                }
        ...
        if let addressComponents = dictionary["address_components"] as? [[String: Any]] {
            for addressComponent in addressComponents {
                guard let types = addressComponent["types"] as? [String],
                      ...
                      let name = addressComponent["long_name"] as? String
                else {
                    continue
                }
                postalTown = name
