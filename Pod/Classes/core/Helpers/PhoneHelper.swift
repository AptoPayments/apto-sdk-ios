...
public class PhoneHelper {
    ...
    public func validatePhoneWith(countryCode: Int? = nil, nationalNumber: String?) -> Bool {
        ...
        let regionCode = region(for: countryCode)
        guard let phoneNumber = try? phoneValidator.parse(nationalNumber, withRegion: regionCode) else {
            return false
        }
    ...
    public func callURL(from phoneNumber: PhoneNumber?) -> URL? {
        guard let phoneNumber = phoneNumber,
              ...
              let number = phoneNumber.phoneNumber.value
        else {
            return nil
        }
