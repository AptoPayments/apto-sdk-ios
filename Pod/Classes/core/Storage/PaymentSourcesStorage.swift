...
struct PaymentSourcesStorage: PaymentSourcesStorageProtocol {
  ...
  func addPaymentSource(_ apiKey: String, userToken: String, _ paymentSource: PaymentSourceRequest, callback: @escaping Result<PaymentSource, NSError>.Callback)
  {
    ...
    transport.post(url, authorization: auth, parameters: self.buildAddPaymentSource(with: paymentSource), filterInvalidTokenResult: true) { result in
      switch result {
  ...
  func getPaymentSources(_ apiKey: String,
    ...
    let url = URLWrapper(baseUrl: transport.environment.baseUrl(),
                         url: JSONRouter.paymentSources,
                         urlParameters: self.buildUrlParametersForPaymentSources(with: request))
    let auth = JSONTransportAuthorization.accessAndUserToken(projectToken: apiKey, userToken: userToken)
    transport.get(url, authorization: auth, parameters: nil, headers: nil, acceptRedirectTo: nil, filterInvalidTokenResult: true) { result in
      switch result {
      case .success(let json):
  ...
  func pushFunds(_ apiKey: String, userToken: String, request: PushFundsRequest, callback: @escaping Result<PaymentResult, NSError>.Callback)
  {
    ...
    let url = URLWrapper(baseUrl: transport.environment.baseUrl().replacingOccurrences(of: "/v1", with: ""), // TODO: Change when backend fix url
                         url: JSONRouter.paymentSourcesPushFunds,
                         urlParameters: [":paymentSourceId": request.paymentSourceId])
    let auth = JSONTransportAuthorization.accessAndUserToken(projectToken: apiKey, userToken: userToken)
    transport.post(url, authorization: auth, parameters: self.buildPushFunds(with: request), filterInvalidTokenResult: true) { result in
      switch result {
      case .success(let json):
  ...
  private func buildAddPaymentSource(with request: PaymentSourceRequest) -> [String: AnyObject]? {
    ...
    case .bankAccount(let bankAccountRequest):
      return [
