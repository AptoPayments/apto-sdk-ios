...
class UserTokenStorage: UserTokenStorageProtocol {
    ...
    private func registerToInvalidSessionEvents() {
        notificationHandler.addObserver(self, selector: #selector(didReceiveSessionExpiredEvent),
                                        name: .UserTokenSessionExpiredNotification)
        notificationHandler.addObserver(self, selector: #selector(didReceiveSessionInvalidEvent),
                                        name: .UserTokenSessionInvalidNotification)
        notificationHandler.addObserver(self, selector: #selector(didReceiveSessionClosedEvent),
    ...
    private func loadPersistedToken() {
        guard let data = keychain.value(for: .tokenKey),
              let token = try? JSONDecoder().decode(UserTokenWithCredentials.self, from: data)
        else {
            return
        }
