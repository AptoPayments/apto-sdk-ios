...
@objc open class UIConfig: NSObject {
    ...
    open lazy var formFieldInternalPadding: CGRect = {
        CGRect(x: 0, y: 0, width: 15, height: self.formRowHeight)
    }()
    ...
    public init(branding: Branding, fontCustomizationOptions: FontCustomizationOptions? = nil) {
        lightBranding = ParsedProjectBranding(branding: branding.light)
        darkBranding = ParsedProjectBranding(branding: branding.dark)
        super.init()
...
private extension UIConfig {
    func setFonts(fontCustomizationOptions: FontCustomizationOptions?) {
        if let fontCustomizationOptions = fontCustomizationOptions {
            ...
            case let .fontDescriptors(descriptors):
                fontProvider = UITheme2FontProvider(fontDescriptors: descriptors)
            case let .fontProvider(provider):
                fontProvider = provider
            }
        } else {
            fontProvider = UITheme2FontProvider(fontDescriptors: nil)
        }
    }
}
