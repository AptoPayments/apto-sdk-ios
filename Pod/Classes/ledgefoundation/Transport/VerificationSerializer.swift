...
extension Verification: JSONSerializable {
  ...
  public func jsonSerialize() -> [String: AnyObject] {
    ...
    data["verification_type"] = (self.verificationType == .email ? "email" : self.verificationType == .phoneNumber ? "phone" : self.verificationType == .birthDate ? "birthdate" : "") as AnyObject
    return data
  }
